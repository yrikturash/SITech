var addColumnFiltersDataTable = function(filtersCount, startPos)
{
	...
	if( $('#datatable-column-filter').length > 0 ) {
		...
		$('#datatable-column-filter thead').append('<tr class="row-filter">'+thstr+'</tr>');
		...
		if (startPos > 0) {
			...
			$('#datatable-column-filter thead .row-filter').prepend(prestr);
		};
...
var addColumnFiltersDataTable = function(id, filtersCount, startPos)
{
	startPos = startPos || 0; // по умолчанию 0
	/* column filters */
	if( $(id).length > 0 ) {
		...
		$(id+' thead').append('<tr class="row-filter">'+thstr+'</tr>');
		$(id+' thead .row-filter th').each( function() {
			$(this).html('<input type="text" class="form-control input-sm" placeholder="Search...">');
		});
		...
		if (startPos > 0) {
			...
			$(id+' thead .row-filter').prepend(prestr);
		};

		$(id+' .row-filter input').on('keyup change', function() {
			dtTable
				.column($(this).parent().index()+':visible')
				.search(this.value)
				.draw();
